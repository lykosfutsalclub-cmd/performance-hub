...
test("le même contrôle d’accès précède le Performance Hub et le eStaff", () => {
  assert.match(home, /access-gate\.js\?v=20260923-global-access-v1/);
  assert.match(estaff, /\.\.\/access-gate\.js\?v=20260923-global-access-v1/);
...
test("le code n’est ni enregistré ni publié dans les fichiers", () => {
  ...
  assert.doesNotMatch(gate, /localStorage\.setItem\([^,]+,\s*code/);
...
test("la session vérifiée est commune aux pages et remplace le second clavier eStaff", () => {
  assert.match(gate, /lykos_performance_hub_session_v1/);
