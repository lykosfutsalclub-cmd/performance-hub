...
- Après l’échec initial et son unique reprise automatique, une étiquette `estaff-sync-suspended/AAAA-MM-JJ` suspend les nouvelles tentatives horaires afin d’éviter une boucle ou des connexions répétées. Elle n’efface pas l’échéance due. Après correction, une relance manuelle ou un commit explicitement marqué `[esupport-sync]` peut reprendre le même créneau et créer sa preuve de réussite.
