...
- Si la seconde tentative échoue, aucune troisième tentative n’est lancée dans la même exécution : l’échec reste visible. Le contrôle horaire suivant reprend cependant l’échéance tant que sa preuve de réussite manque. Cette limite empêche une boucle infinie dans une même exécution sans créer de blocage persistant, et ne contourne jamais les tests, le GO de Véronique, la confidentialité ou la sécurité.
...
- Le contrôle horaire sert aussi de filet de rattrapage lorsqu’un créneau quotidien n’a jamais été créé par GitHub. L’échéance reste due, y compris après minuit, jusqu’à la présence d’une étiquette Git `estaff-sync-proof/AAAA-MM-JJ`. Cette preuve n’est créée qu’après collecte, reconstruction, contrôles, éventuelle publication, vérification publique, contrôle privé et journalisation réussis. Une étiquette Git est ici un repère versionné public ; elle ne contient aucune donnée sportive ni aucun secret.
- Le même repère empêche le créneau d’été, le créneau d’hiver ou un contrôle horaire retardé de rejouer une journée déjà entièrement exécutée. Un échec ne crée jamais la preuve : l’échéance reste donc visible comme due et la publication demeure soumise à tous les contrôles existants.
- Aucun état de suspension persistant n’est créé après un échec. Lorsque des identifiants ou un service redeviennent disponibles, le contrôle horaire suivant reprend donc automatiquement le créneau encore dû, sans commit ni relance humaine.
