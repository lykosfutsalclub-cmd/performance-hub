(() => {
  ...
  const state = {
    ...
    scenarios: safeJSON(localStorage.getItem('lykos_fm_lineup_scenarios_v1'), { A: null, B: null }),
    matchDraft: safeJSON(localStorage.getItem('lykos_fm_match_dossier_v1'), {}),
  };
  ...
  function lineupMetrics(lineup = state.lineup) {
    const selected = lineup.map((id) => playerMap.get(id)).filter(Boolean);
    const average = (key) => {
      const values = selected.map((player) => player[key]).filter((value) => value != null && Number.isFinite(Number(value)));
      return values.length ? values.reduce((sum, value) => sum + Number(value), 0) / values.length : null;
    };
    return {
      count: selected.length,
      metron: average('metron'),
      creation: average('creation'),
      finishing: average('finishing'),
      defensive: average('defensive'),
      contributionsPerGame: selected.reduce((sum, player) => sum + (player.contributionsPerGame || 0), 0),
    };
  }
  function scenarioSnapshot() {
    return { formation: state.formation, lineup: [...state.lineup], savedAt: new Date().toISOString() };
  }
  function scenarioCard(key) {
    const scenario = state.scenarios[key];
    if (!scenario) return `<article class="fm-scenario-card is-empty"><span>Scénario ${key}</span><strong>Non enregistré</strong><small>Composez un cinq puis enregistrez-le ici.</small><button class="fm-button" data-save-scenario="${key}">Enregistrer le cinq actuel</button></article>`;
    const metrics = lineupMetrics(scenario.lineup || []);
    const names = (scenario.lineup || []).map((id) => playerMap.get(id)?.name).filter(Boolean);
    return `<article class="fm-scenario-card"><span>Scénario ${key} · ${esc(scenario.formation)}</span><strong>${names.join(' · ') || 'Composition incomplète'}</strong><div class="fm-scenario-metrics"><small>Metron <b>${num(metrics.metron, 1)}</b></small><small>Création <b>${num(metrics.creation, 1)}</b></small><small>Finition <b>${num(metrics.finishing, 1)}</b></small><small>Défense <b>${num(metrics.defensive, 1)}</b></small></div><div class="fm-actions"><button class="fm-button" data-load-scenario="${key}">Charger</button><button class="fm-button" data-save-scenario="${key}">Remplacer</button></div></article>`;
  }
  function formHTML() {
  ...
  function renderLineup() {
    ...
    const scenarioA = state.scenarios.A ? lineupMetrics(state.scenarios.A.lineup) : null;
    const scenarioB = state.scenarios.B ? lineupMetrics(state.scenarios.B.lineup) : null;
  ...
  function renderMatchOverview() {
    ...
    const hasDraft = Boolean(draft.opponent || draft.date || draft.competition);
    const dossier = `<form class="fm-match-dossier" data-match-dossier><div class="fm-dossier-head"><div><span>Brouillon local · à confirmer</span><h3>${hasDraft ? esc(draft.opponent || 'Adversaire à confirmer') : 'Préparer le prochain match'}</h3><p>Ces informations restent sur cet appareil et ne deviennent jamais une donnée officielle du Performance Hub.</p></div><div class="fm-brand-placeholder">?</div></div><div class="fm-dossier-fields"><label>Adversaire<input name="opponent" value="${esc(draft.opponent || '')}" placeholder="Nom du club"></label><label>Date<input name="date" type="date" value="${esc(draft.date || '')}"></label><label>Heure<input name="time" type="time" value="${esc(draft.time || '')}"></label><label>Lieu<input name="location" value="${esc(draft.location || '')}" placeholder="Salle / ville"></label><label>Compétition<input name="competition" value="${esc(draft.competition || '')}" placeholder="Championnat, coupe…"></label><label>Importance<select name="importance"><option value="">À définir</option>${['Faible','Normale','Élevée','Décisive'].map((value) => `<option${draft.importance === value ? ' selected' : ''}>${value}</option>`).join('')}</select></label><label class="is-wide">Notes<textarea name="notes" rows="3" placeholder="Enjeu, consignes, informations utiles…">${esc(draft.notes || '')}</textarea></label></div><div class="fm-actions"><button class="fm-button is-primary" type="submit">Enregistrer le dossier</button><button class="fm-button" type="button" data-clear-match-draft>Effacer</button><button class="fm-button" type="button" data-go="match" data-sub="preparation">Ouvrir la préparation</button></div></form>`;
    return `<div class="fm-grid"><div class="fm-col-12">${panel('Prochaine rencontre', dossier, info('Brouillon de match', 'Le dossier permet d’avancer même lorsque la synchronisation calendrier manque. Sa mention « à confirmer » évite de le confondre avec une donnée SportEasy vérifiée.'))}</div><div class="fm-col-7">${panel('Dernière rencontre', latestFixture ? `<div class="fm-match-hero"><div class="fm-team"><img src="../logo-lykos-intro-carre-2026.png" alt=""><strong>Lykos FC</strong></div><div class="fm-score"><strong>${latestFixture.scoreFor}–${latestFixture.scoreAgainst}</strong><small>${dateFR(latestFixture.date)} · ${fixtureResult(latestFixture)}</small></div><div class="fm-team"><div class="fm-brand-placeholder" style="margin:auto">?</div><strong>${esc(latestFixture.opponent)}</strong></div></div>` : empty('Aucun match', 'Aucun résultat disponible.'))}</div><div class="fm-col-5">${panel('Repères de forme', `${formHTML()}<div class="fm-kpis" style="margin-top:14px">${kpi('Victoires', num(CURRENT.wins, 0), `${num(CURRENT.winRate, 1)} %`, 'positive')}${kpi('Défaites', num(CURRENT.losses, 0), 'saison actuelle', 'negative')}</div>`)}</div><div class="fm-col-12">${panel('Repères de la saison', `<div class="fm-kpis">${kpi('Meilleur buteur', scorer ? scorer.name : '—', `${scorer?.goals || 0} buts`)}${kpi('Meilleur passeur', passer ? passer.name : '—', `${passer?.assists || 0} passes`)}${kpi('Metron moyen', num(metronAverage, 1), `${metronValues.length} joueurs notés`)}${kpi('Indisponibles', '—', 'source non branchée')}</div>`)}</div></div>`;
  }
  ...
  function renderPreparation() {
    const draft = state.matchDraft || {};
    const target = draft.opponent ? `contre ${esc(draft.opponent)}` : 'sans adversaire confirmé';
    return `<div class="fm-grid"><div class="fm-col-8">${panel('Préparation de match', `<div class="fm-status-strip"><strong>${target}</strong><span>${draft.date ? `${dateFR(draft.date)}${draft.time ? ` · ${esc(draft.time)}` : ''}` : 'Date à confirmer'} · brouillon local</span></div><div class="fm-callout" data-tone="warning" style="margin-top:10px"><b>Le dossier facilite la préparation mais ne remplace pas une synchronisation SportEasy.</b> Toute information locale reste signalée « à confirmer ».</div><ul class="fm-list" style="margin-top:10px"><li><i></i><div><strong>Valider les disponibilités</strong><small>À contrôler depuis la source de présence.</small></div><b>À faire</b></li><li><i></i><div><strong>Comparer les cinq possibles</strong><small>Deux scénarios peuvent être enregistrés et comparés.</small></div><button class="fm-button" data-go="lineup">Comparer</button></li><li><i></i><div><strong>Fixer le groupe</strong><small>${state.group.length} joueur${state.group.length > 1 ? 's' : ''} dans le groupe local.</small></div><button class="fm-button" data-go="match" data-sub="group">Ouvrir</button></li><li><i></i><div><strong>Analyser l’adversaire</strong><small>${draft.opponent ? `Dossier créé pour ${esc(draft.opponent)}, rapport vidéo encore absent.` : 'Nécessite un adversaire identifié.'}</small></div><button class="fm-button" disabled>Rapport absent</button></li></ul>`)}</div><div class="fm-col-4">${panel('Regards eStaff', `<ul class="fm-list"><li><i></i><div><strong>Informations de Victor</strong><small>« Les ${players.length} disponibilités doivent être confirmées avant la sélection. »</small></div><button class="fm-info" data-info-title="Informations de Victor" data-info="Constat : aucune source de disponibilité fiable n’est branchée. Conséquence : aucun joueur n’est déclaré disponible par défaut.">i</button></li><li><i></i><div><strong>Analyse de Giannis</strong><small>« La forme collective est positive, avec un échantillon encore court. »</small></div><button class="fm-info" data-info-title="Analyse de Giannis" data-info="La note de forme publiée est ${num(CURRENT.form?.value, 1)} sur ${CURRENT.form?.matchCount || 0} matchs, avec ${num(CURRENT.winRate, 1)} % de victoires.">i</button></li><li><i></i><div><strong>Avis de Léonard</strong><small>« Comparez les scénarios avant d’arrêter le cinq. »</small></div><button class="fm-info" data-info-title="Avis de Léonard" data-info="Le comparateur confronte Metron, création, finition et défense. Il éclaire le choix sans décider à la place du coach.">i</button></li><li><i></i><div><strong>Rapport de Nadir</strong><small>Vidéo du prochain adversaire non disponible.</small></div><button class="fm-info" data-info-title="Rapport de Nadir" data-info="Aucun document vidéo associé n’est disponible dans la source publique.">i</button></li></ul><a class="fm-button" href="../estaff/">Ouvrir l’eStaff</a>`)}</div></div>`;
  }
  ...
  function navigate(route, sub = null, push = true) {
    ...
    $('[data-more-menu]')?.setAttribute('hidden', '');
    $('[data-more-toggle]')?.setAttribute('aria-expanded', 'false');
  ...
  document.addEventListener('click', (event) => {
    const moreToggle = event.target.closest('[data-more-toggle]'); if (moreToggle) { const menu = $('[data-more-menu]'); const open = menu.hasAttribute('hidden'); menu.toggleAttribute('hidden', !open); moreToggle.setAttribute('aria-expanded', String(open)); return; }
    ...
    const saveScenario = event.target.closest('[data-save-scenario]'); if (saveScenario) { const key = saveScenario.dataset.saveScenario; state.scenarios[key] = scenarioSnapshot(); localStorage.setItem('lykos_fm_lineup_scenarios_v1', JSON.stringify(state.scenarios)); render(); return toast(`Scénario ${key} enregistré.`); }
    const loadScenario = event.target.closest('[data-load-scenario]'); if (loadScenario) { const scenario = state.scenarios[loadScenario.dataset.loadScenario]; if (scenario) { state.formation = FORMATIONS[scenario.formation] ? scenario.formation : state.formation; state.lineup = [...scenario.lineup]; render(); return toast(`Scénario ${loadScenario.dataset.loadScenario} chargé.`); } }
    const clearScenarios = event.target.closest('[data-clear-scenarios]'); if (clearScenarios) { state.scenarios = { A: null, B: null }; localStorage.removeItem('lykos_fm_lineup_scenarios_v1'); render(); return toast('Comparateur réinitialisé.'); }
    const clearMatchDraft = event.target.closest('[data-clear-match-draft]'); if (clearMatchDraft) { state.matchDraft = {}; localStorage.removeItem('lykos_fm_match_dossier_v1'); render(); return toast('Brouillon de match effacé.'); }
  ...
  document.addEventListener('submit', (event) => {
    const form = event.target.closest('[data-match-dossier]');
    if (!form) return;
    event.preventDefault();
    state.matchDraft = Object.fromEntries(new FormData(form).entries());
    localStorage.setItem('lykos_fm_match_dossier_v1', JSON.stringify(state.matchDraft));
    render();
    toast('Dossier local enregistré · informations à confirmer.');
  });
